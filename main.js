...
function display_menu(){
    for (i = 0; i < categories.length; i++){
        ...
        $("#home_page").append("<div id='"+categories[i]+"'></div>"); //make div for category
        $("#"+categories[i]).append("<b>"+categories[i] + "</b><br>"); //print category name in div
        for (let j = 0; j < relevant_exos.length; j++) { //print exos in div
            $("#"+categories[i]).append('<div id="'+relevant_exos[j].name+'" onclick="select_exo(\''+relevant_exos[j].name +'\')">'+relevant_exos[j].name+'</div>'); 
        }
        $("#home_page").append("<br>"); 
    };
};
